...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"payzchannel\")"
  ...
  echo
  echo "	byfn.sh generate -c payzchannel"
  echo "	byfn.sh up -c payzchannel -s couchdb"
  echo "        byfn.sh up -c payzchannel -s couchdb -i 1.4.0"
  echo "	byfn.sh up -l javascript"
  echo "	byfn.sh down -c payzchannel"
  echo "        byfn.sh upgrade -c payzchannel"
  echo
...
function networkUp() {
  ...
  docker exec cli scripts/ci.sh $CHANNEL_NAME $CLI_DELAY $CC_SRC_LANGUAGE $CLI_TIMEOUT $VERBOSE $NO_CHAINCODE
  if [ $? -ne 0 ]; then
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    ...
    if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
      ...
      export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org1.payzchain.com/ca && ls *_sk)
      export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org2.payzchain.com/ca && ls *_sk)
    fi
...
CLI_DELAY=3
# channel name defaults to "payzchannel"
