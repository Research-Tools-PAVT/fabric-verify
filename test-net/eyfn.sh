...
function printHelp () {
  ...
  echo
  echo "	eyfn.sh generate -c payzchannel"
  echo "	eyfn.sh up -c payzchannel -s couchdb"
  echo "	eyfn.sh up -l javascript"
  echo "	eyfn.sh down -c payzchannel"
  echo
...
function removeUnwantedImages() {
  DOCKER_IMAGE_IDS=$(docker images|awk '($1 ~ /dev-peer.*.marblescc.*/) {print $3}')
